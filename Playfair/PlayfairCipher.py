...
class Playfair:
  ...
  def encrypt(self, key, cleartext):

    key = key.lower()
    cleartext = self.preprocessText(cleartext)
    matrix = self.generateMatrix(key)
    indexDict = dict()
    ciphertext = ""
...
# test bench
cipher = Playfair()
ciphertext = cipher.encrypt("YoanPinzon", "TELL ME, O MUSE, of that ingenious hero who traveled far and wide after he had sacked the famous town of Troy")
cleartext = cipher.decrypt("YoanPinzon", ciphertext)
print(ciphertext)
print(cleartext)
